...
int is_recently_dropped(int passenger_id, int droppedThisTurnCount, int droppedThisTurn[1000]) {
    for (int i = 0; i < droppedThisTurnCount; i++) {
        if (droppedThisTurn[i] == passenger_id) {
            return 1;
        }
    }
    return 0;
...
int main(int argc, char* argv[])
{
    ...
    int zones=5;
    int elev_zones[n][2];
    int floors_per_zone = k / zones;
    int reached_zone[n];
    memset(reached_zone, 0, sizeof(reached_zone));
    for (int i = 0; i < zones; i++) {
        elev_zones[i][0] = i * floors_per_zone; 
        elev_zones[i][1] = (i + 1) * floors_per_zone; 
    ...
    while(1)
    {
        int droppedThisTurn[1000];
        int droppedThisTurnCount = 0;
        memset(droppedThisTurn,0,sizeof(droppedThisTurn));
        for(int i=0;i<n;i++)
        {
            if(elev_zones[i][0]<=shmptr->elevatorFloors[i]<=elev_zones[i][1])
            {
                reached_zone[i]=1;
            }
        }
...
printf("Received TurnChangeResponse\n");
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < soft_limit; j++) {
                int zone_start = elev_zones[i][0];
                int zone_end = elev_zones[i][1];

                if (going_to[i][j][0] != -1 &&
                    (going_to[i][j][1] == shmptr->elevatorFloors[i] ||
                    shmptr->elevatorFloors[i] == zone_start || 
                    shmptr->elevatorFloors[i] == zone_end) &&
                    going_to[i][j][3] == 1) {

                    int person_id = going_to[i][j][0];
                    int destination_floor = going_to[i][j][1];
                    droppedThisTurn[droppedThisTurnCount++] = person_id;
                    PassengerRequest drop_req;
                    drop_req.requestedFloor=destination_floor;
                    drop_req.requestId=person_id;
                    drop_req.startFloor=shmptr->elevatorFloors[i];
                    shmptr->droppedPassengers[dropped++] = person_id;
                    printf("person %d dropped at %d\n", person_id, shmptr->elevatorFloors[i]);
                    total_dropped_off++;

                    if (shmptr->elevatorFloors[i] != destination_floor) {
                        enqueue(&queue, drop_req);
                        printf("Queued new request for person %d to floor %d\n", person_id, destination_floor);
                    }
        ...
        for(int i=0;i<queue.size;i++)
        {
            ...
            int min_distance=k;
            int best_ele=-1;
            if(passenger_already_assigned(pass_id, n, going_to) || is_recently_dropped(pass_id, droppedThisTurnCount, droppedThisTurn)) {
        continue;
    }
            ...
            for (int j=0;j<n;j++)
            {
                if(elev_zones[j][0]<=start_floor<=elev_zones[j][1] && num_people[j]<soft_limit)
                {
                    best_ele=j;
                }
                else
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (reached_zone[i] == 1) {
                ...
                if (num_people[i] != 0) {
                    if (dir[i][0] > dir[i][1] && shmptr->elevatorFloors[i] < zone_end) {
                        shmptr->elevatorMovementInstructions[i] = 'u';
                        moved[i] = 1;
                    } else if (dir[i][0] < dir[i][1] && shmptr->elevatorFloors[i] > zone_start) {
